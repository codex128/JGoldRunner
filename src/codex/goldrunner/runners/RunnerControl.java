...
public abstract class RunnerControl extends AbstractControl implements Traveller, ItemCarrier {
    ...
    private static final Quaternion YAW_180 = new Quaternion().fromAngles(0, FastMath.TWO_PI, 0);
    ...
    protected void controlUpdate(float tpf) {
        ...
        pickup();
        if (occupy.getLast().killer() && isAlive()) {
            kill();
        }
    ...
    public boolean climb() {
        ...
        if (up != null) {
            if (move(up, UnitControl.U, true)) {
                spatial.setLocalRotation(getCurrentFace().getLocalRotation().add(YAW_180));
    ...
    public void pickup() {
        ...
        if (item != null) {
            item.attach(this);
            if (this.item != null) {
                this.item.releaseParticleEffect();
            }
        }
    }
