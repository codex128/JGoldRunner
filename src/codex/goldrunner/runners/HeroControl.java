...
//		push(start);
    ...
    protected void initHUD(AssetManager assets) {
        ...
        if (SettingsState.getInstance().boostingEnabled()) {
            hud.attachChild(energybar);
        }
    }
...
//	}
    ...
    public void dig(int direction) {
        ...
        if (dashing || !getCurrentFace().isGravityInfluenced()) {
            return;
        }
    ...
    public void onAnalog(String name, float value, float tpf) {
        if (!inTransition()) {
            switch (name) {
                case "up":
                    action(transformMovementDirection(UnitControl.U));
                    break;
                case "right":
                    action(transformMovementDirection(UnitControl.R));
                    break;
                case "down":
                    action(transformMovementDirection(UnitControl.D));
                    break;
                case "left":
                    action(transformMovementDirection(UnitControl.L));
                    break;
            }
        }
    }
    ...
    public void onAction(String name, boolean isPressed, float tpf) {
        switch (name) {
            ...
            case "a":
                ...
                break;
            case "space":
                if (isPressed && SettingsState.getInstance().boostingEnabled()
                        && !dashing && energy >= 100) {
                    dashing = true;
...
//		hero.initAudio(assets);
    ...
    /**
     * A simple wrapping method. Does not work if value moves outside
     * (max-min)*2 bounds on either side.
     *
     * @param value
     * @param min
     * @param max
