...
public class ShatterFactory {
    ...
    /**
     * Generate shard geometries matching the triangles composing the asserted
     * geometry.
     * <p>
    ...
    public static Node shatter(Geometry geometry, int maxShards) {
        ...
        for (int index = 0; index < geometry.getMesh().getTriangleCount(); index++) {
            ...
            // shard limit
            if (maxShards >= 0 && index >= maxShards) {
                break;
            }
        }
    ...
    /**
     * Retrieves a Geometry from the asserted spatial. If the spatial is a Node,
     * will retrieve a Geometry from its descendents (if able).
     *
