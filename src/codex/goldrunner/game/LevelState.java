...
public class LevelState extends BaseAppState implements ActionListener,
    ...
    public static final int SOUTH = 0, WEST = 1, NORTH = 2, EAST = 3, UP = 4;
    ...
    protected void initialize(Application app) {
        ...
        // lighting
        scene.addLight(new DirectionalLight(new Vector3f(1, -1, -1), ColorRGBA.White.mult(0.5f)));
        //scene.addLight(new DirectionalLight(new Vector3f(-1, 1, 1), ColorRGBA.White.mult(0.2f)));
...
//        }
        ...
        // light probe
        Spatial probeModel = app.getAssetManager().loadModel("Scenes/lagoon-probe.j3o");
        scene.addLight(probeModel.getLocalLightList().get(0));
    ...
    public void update(float tpf) {
        if (!hero.isAlive() && hero.isEnabled() && !endtimer.isRunning()) {
            endtimer.start();
            for (RunnerControl runner : runners) {
                runner.pause(true);
            }
        ...
        if (faces != null) {
            for (MapFace face : faces) {
                if (bullet != null && bullet.getPhysicsSpace() != null) {
                    for (int i = 0; i < face.getMap().length; i++) {
    ...
    private void loadBorderConcrete(int thickness, int height) {
        ...
        if (loader != null) {
            for (int j = 0; j < thickness; j++) {
                for (int i = -(height-1)+j+1; i <= faces[0].map.length+thickness-1; i++) {
                    ...
                    c2.setLocalTranslation(getMapLocation(faces[0].map[0].length+j, i, 0f));
                    level.attachChild(c1);
                    level.attachChild(c2);
                }
            }
            for (int i = 0; i < faces[0].map[0].length; i++) {
                for (int j = 0; j < thickness; j++) {
                    Spatial c2 = loader.loadSpatial("concrete", false, assets);
                    c2.setLocalTranslation(getMapLocation(i, faces[0].map.length+j, 0f));
                    level.attachChild(c2);
                }
            }
        }
...
//		}
    ...
    private void createWorldBackground(float depth) {
        ...
        front.setLocalTranslation(getMapLocation(
                        faces[0].map[faces[0].map.length-1].length/2, faces[0].map.length, -distance));
        ...
        fm.setColor("Diffuse", new ColorRGBA(.05f, .05f, .05f, 1f));
    ...
    private Vector3f getFaceTranslation(int index) {
        switch (index) {
            case 1:
                return new Vector3f(-UNIT_SIZE, 0f, -(faces[1].getMap()[0].length) * UNIT_SIZE);
            case 2:
                return new Vector3f((faces[0].getMap()[0].length - 1) * UNIT_SIZE, 0f, -(faces[1].getMap()[0].length + 1) * UNIT_SIZE);
            case 3:
                return new Vector3f(faces[0].getMap()[0].length * UNIT_SIZE, 0f, -UNIT_SIZE);
            case 4:
                return new Vector3f(0f, -TOP_MARGIN * UNIT_SIZE, -(faces[4].getMap().length) * UNIT_SIZE);
            default:
                return new Vector3f();
        }
    }
    ...
    private Quaternion getFaceRotation(int index) {
        switch (index) {
            case 1:
                return new Quaternion().fromAngleAxis(-FastMath.PI * 0.5f, Vector3f.UNIT_Y);
            case 2:
    ...
    public static UnitLoader getUnitLoader(String type) {
        if (type == null) {
            return loaders.get(DEFAULT_LOADER);
        }
